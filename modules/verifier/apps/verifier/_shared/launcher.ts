/**
 * Tmux + new-OS-window launcher for the Verifier Agent (`$TMUX`-aware).
 *
...
export interface SpawnOpts {
  ...
  sessionId: string;
  /** Absolute path to `.mouthpeace/verifier/agents/verify_<domain>.md`. */
  agentPath: string;
  /** Absolute path to repo root, used to resolve `apps/verifier/verifier.ts`. */
  runtimeRoot: string;
  /** `ctx.cwd` — used for `.mouthpeace/state/` breadcrumb resolution. */
  cwd: string;
  /** `.mouthpeace/settings.json` — only `verifier.terminalCommand` is consulted. */
  settings?: { verifier?: { terminalCommand?: string } };
  /** Absolute path to `~/.mouthpeace/agent/sessions/<sid>.jsonl` — fed into `<BUILDER_SESSION_FILE>`. */
...
export interface SpawnResult {
  ...
  wrapperPath: string;
  /**
   * File the wrapper redirects the agent's stderr into (mirrored to the terminal
   * via `tee` so the tmux pane still shows it). Read on spawn-failure to
   * surface the agent's actual error in the builder — far more useful than the
...
export async function spawnVerifierChild(opts: SpawnOpts): Promise<SpawnResult> {
  ...
  //
  // The harness defaults to its full tool set; we forward only what the persona's
...
interface BuildSpawnWrapperOpts {
  ...
  tools: string;
  /**
   * Persona's `model:` frontmatter value, passed to the harness via `--model` so the
   * verifier child uses the persona-declared model regardless of what the
   * user's harness config has selected globally. Without this, a user running
   * deepseek-v4-pro as their default would get a deepseek verifier even when
   * the persona declares a different model.
   */
  model: string;
  /**
   * File to capture the agent's stderr into. The wrapper redirects stderr through
   * `tee`, so the file gets the agent's actual error output (e.g. "model not
   * found") AND the tmux pane still shows it. The
...
function buildSpawnWrapper(opts: BuildSpawnWrapperOpts): string {
  ...
  //   record the agent's exit code into the stderr log. With `set -e` + `exec`,
  //   the wrapper would die mid-pipeline and the builder's diagnosis
  //   would only ever see whatever the agent flushed before the kill.
  //
  // - Stderr is redirected through `tee -a "$STDERR_LOG" >&2` (bash
  //   process substitution). That writes the agent's stderr to the log file
  ...
  //   the agent's interactive TUI uses the terminal directly (ioctl on
  ...
  return [
    "#!/usr/bin/env bash",
    "# Auto-generated by Verifier launcher.ts.",
    "# Sets up env, runs the verifier child, captures stderr to a log.",
    ...
    // the stderr capture would silently miss the agent's actual output.
    "pi \\",
    ...piArgs.map((a) => `  ${a} \\`),
    ...
    "EXIT_CODE=$?",
    `echo "" >> "$STDERR_LOG"`,
    `echo "[wrapper] agent exited with code $EXIT_CODE" >> "$STDERR_LOG"`,
    "exit $EXIT_CODE",
    "",
  ].join("\n");
}
 ...
 * `.mouthpeace/settings.json` to a template that pipes through their own tmux config,
 ...
 * string with no whitespace (matches the harness's `--tools` flag format), and ALWAYS
 ...
 * missing from `--tools`, the harness filters it out and the LLM sees "tool not
...
async function openOsWindowAttachedTo(
  ...
  // emulator can set `verifier.terminalCommand` in .mouthpeace/settings.json.
...
async function tryDispatchMacOS(
  ...
  try {
    switch (term) {
      ...
      case "ghostty":
      case "Ghostty":
        // Ghostty's `-e` expects an executable and argv. Passing
        // `"tmux attach -t ..."` as one string makes Ghostty try to exec a
        // literal file with spaces in its name, which opens a failure window.
        if (await commandExists("ghostty")) {
          await execFileP("ghostty", ["-e", "tmux", "attach", "-t", tmuxSession]);
        } else {
