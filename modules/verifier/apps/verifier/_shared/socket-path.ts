/**
 * Socket-path resolution for the Verifier Agent.
 *
 * Layout:
 *   actual socket : /tmp/pi-verifier/<sessionId>.sock
 *   breadcrumb    : <cwd>/.mouthpeace/state/verifier-<sessionId>.sock.ref   (plain text → socket path)
 *
 * Why `/tmp/pi-verifier/`:
 ...
 *   (e.g. `/Users/foo/Documents/projects/the-verifier-agent/.mouthpeace/state/verifier-<sid>.sock`)
 *   blows past 104 bytes for almost every realistic project location.
 *   `/tmp/pi-verifier/<sid>.sock` is short and predictable.
 *
 * The breadcrumb file in the project lets `ls .mouthpeace/state/` describe what
...
export function resolveSocketPath(sessionId: string, cwd: string): ResolvedSocketPaths {
  ...
  const refPath = path.join(cwd, ".mouthpeace", "state", `verifier-${sessionId}.sock.ref`);
 ...
 *
 * Creates the `.mouthpeace/state/` parent dir if needed (so callers don't need
