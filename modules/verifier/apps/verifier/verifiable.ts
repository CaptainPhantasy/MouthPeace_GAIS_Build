/**
 * Builder-side extension for the Verifier Agent.
 *
...
export default function verifiable(pi: ExtensionAPI): void {
  ...
  pi.registerFlag("verifier-agent", {
    type: "string",
    description:
      "Persona name under .mouthpeace/verifier/agents/ (without .md). Default: verifier.",
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    state.sessionFilePath =
      sessionFile ?? path.join(os.homedir(), ".mouthpeace/agent/sessions", `${state.sessionId}.jsonl`);
    ...
    // Replace the harness's default input editor with one that embeds verifier
    // status / model / ctx info into the input box's border lines.
    // See `verifiable-footer.ts` for implementation details.
    if (ctx.hasUI) {
      ...
      // Hide the harness's default footer entirely. The input-bar borders already
      // carry model + ctx % + verifier-status; the default footer (model +
  ...
  pi.on("session_shutdown", async (_event, ctx) => {
    // Restore the harness's default editor + footer on shutdown so a subsequent
    // invocation (without --verifiable) still sees a clean baseline.
    if (ctx?.hasUI) {
      try {
  ...
  async function attach(ctx: ExtensionContext | ExtensionCommandContext): Promise<void> {
    ...
    try {
      ...
      // .mouthpeace/verifier/agents/ relative to the project cwd.
      ...
      // can read the agent's actual stderr AND tell the user how to reproduce the
   ...
   * async boundaries (the harness marks it stale on internal session replacement),
   ...
   * need to nudge the harness explicitly.
   */
  function setFooter(
    _ctx: ExtensionContext,
    ...
    state.phase = label;
    // Nudge the harness to re-render so the input-bar border picks up the new phase
   ...
   * 0 if the file doesn't exist yet (the harness creates it on first write — for the
  ...
  async function currentSessionFileLineCount(sessionPath: string): Promise<number> {
    ...
    try {
      ...
      // Count `\n` bytes. JSONL entries always end with a newline (the harness
