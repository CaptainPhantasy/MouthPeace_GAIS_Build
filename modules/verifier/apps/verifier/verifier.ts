/**
 * Verifier Agent — verifier-side extension.
 *
 * Loaded into the *child* agent harness instance that runs inside the tmux window the
 * launcher created. The default-export factory is invoked by the harness at startup
 ...
 *      template `.mouthpeace/verifier/prompts/verify_on_stop.md` and inject it via
...
export default function verifierExtension(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event: SessionStartEvent, ctx) => {
    ...
    // uppercase. e.g. ".mouthpeace/verifier/agents/verifier.md" → "VERIFIER".
    ...
    // custom footer (overrides the harness's default — we only show what's relevant
...
/**
 * Install the verifier's minimal footer. Replaces the harness's default footer entirely
